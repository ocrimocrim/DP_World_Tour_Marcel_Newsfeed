...
def _fetch_with_curl_cffi(timeout: int, attempts: int) -> str:
    ...
    for attempt in range(1, attempts + 1):
        impersonate, user_agent = random.choice(_CURL_IMPERSONATIONS)
        headers = _build_headers(user_agent)
        try:
            warmup = curl_requests.get(
                BASE_URL + "/",
            ...
            cookies = warmup.cookies
            if warmup.status_code >= 400:
                LOGGER.debug(
                    "Warm-up request for %s returned %s", BASE_URL, warmup.status_code
                )
